...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      <div>
        <h1 className='rounded-lg mb-4 p-4 bg-white'>Manage all your schools from here!</h1>
        {loading ? (
          <p className='flex justify-center bg-white p-4 rounded-lg'>loading...</p>
        ) : (
            ...
            {mySchools.length > 0 ? (
              <>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <Link href="/school-mgmt/add" className='bg-blue-500 px-2 py-2 rounded'>+ school</Link>
                </div>
                <ul className='flex flex-col gap-2'>
                  {mySchools?.map((school: School, index) => (
                    <li key={index} className='p-2 rounded-md hover:bg-blue-200 hover:cursor-not-allowed flex justify-between'>
