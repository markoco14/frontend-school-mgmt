...
class ClassStudentAdapter {
  public async list({
    ...
    if (class_id)
      queryParams.push(`school_class=${encodeURIComponent(class_id)}`);
  ...
  public async add({
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/class-students/`,
      {
        method: "POST",
        headers: {
  ...
  public async delete({
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/classes/${class_id}/students/${student_id}/delete/`,
      {
        method: "DELETE",
      },
    );
